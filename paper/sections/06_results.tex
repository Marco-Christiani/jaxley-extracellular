...
This section reports four results that bear on whether the simulator
can be trusted for stimulation design: agreement with NEURON on a
realistic morphology, threshold amplitudes across distance and pulse
width, the cell's response across the asymmetric biphasic waveform
space at fixed charge per pulse, and infrastructure scaling for
design-space sweeps that NEURON cannot run in the same time budget.
We close with a gradient receipt that motivates the next step:
direct optimisation of the waveform itself. All runs reported below are part
...
$34\,^\circ$C to match. (iii) NEURON's \texttt{geom\_nseg} and Jaxley's
\texttt{ncomp}+\texttt{max\_branch\_len} knobs settle on different
grids on this morphology ($1131$ vs $700$; \cref{sec:cells-bbp}); we
report the mismatch rather than forcing them to match.
...
counts agree to $\pm 2$ over $\sim\!50$ spikes per cell (Pyr: $16$
NEURON / $18$ Jaxley; PV: $50$/$51$), and AP peak heights agree to
...
the latter is dominated by spike-edge timing offsets. The pre-stim Jaxley
trace runs about $0.4\text{--}0.5\,$mV more hyperpolarised than NEURON's,
attributable to our Euclidean-distance approximation under-applying
the apical $I_h$ gradient distally (\cref{sec:cells-bbp}); after the
stim runs and the cell settles, the residual narrows to roughly
$0.1\,$mV. A path-distance traversal would close it further.

\paragraph{Step 3: full-morphology Pyr, extracellular.} The central
empirical claim of this section is that ECS works on a realistic
branched cell.
...
sub-millivolt residuals on a realistic morphology, with enough
fidelity to support design-space sweeps that NEURON cannot run in the
same time budget (\cref{sec:res-throughput}).
...
The strength--duration curve gives the smallest amplitude that
evokes a spike at each electrode distance and pulse width, and it
underlies many downstream design decisions~\cite{4122088}. We define the threshold $A^*$ for a fixed
...
$(d,\,\text{polarity},\,t_p)$ triple, with four electrode distances
...
(\cref{fig:strength-duration}). Monophasic anodic resolves above the bracket
ceiling only at short pulse widths, with the viable pulse-width range
broadening as electrode distance grows (one width at $d = 25\,\mu$m;
five widths at $d = 200\,\mu$m); longer pulses saturate at the bracket
ceiling. This polarity asymmetry is the discrete analogue of the
activating-function sign argument: cathodic stimulation sets up a
depolarising forcing in the central segments where the cable fires
first, whereas anodic stimulation depolarises the cable ends, which
only triggers under brief, geometrically-favourable
conditions~\cite{rattay1986analysis,Mcneal1976AnalysisOA}. For a
designer choosing parameters, cathodic and biphasic thresholds track
each other closely across the sweep (median biphasic/cathodic ratio
$\approx 1.1$), with biphasic occasionally lower at long pulse widths
and the farther distances; pure anodic stimulation is viable only in
...
Asymmetric biphasic pulses vary activation selectivity and charge
efficiency at fixed delivered charge per pulse; we map the responding
region of the $(T_p, T_n)$ plane. A symmetric charge-balanced biphasic pulse has
...
implication for waveform design is direct: asymmetry is not a
marginal parameter. On this cell, at this distance, swapping a
symmetric biphasic pulse for an asymmetric one with the same charge
can be the difference between sub-threshold and reliable spiking.
...
the electrode along $+y$ (away from the soma, toward the apical tree)
reduces the soma response, consistent with the $1/d$ decay in the
field model; the
