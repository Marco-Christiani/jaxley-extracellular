...
For a uniform straight cable with a point-source electrode,
$[G\boldsymbol{\phi}]_j$ at interior compartments is a fixed multiple
of $\partial^2\phi/\partial x^2$, with the prefactor absorbing
Jaxley's per-row capacitance/area normalisation and unit conversions
(\cref{sec:method}). We compare $G\boldsymbol{\phi}$ against this
analytical reference, evaluated with the matching prefactor, on the
central $10\,\%$ of the cable (boundary effects dominate near the
ends)
...
\]
returned in $37.6\,$s on a v5e single chip. The position gradient is
interpreted physically in \cref{sec:res-grad}; the receipt is part of
the artifact set.
