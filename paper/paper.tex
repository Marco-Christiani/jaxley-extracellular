...
entries $\tilde g_{jk} = g_{jk} / (c_j A_j)$ (in $1/$ms), so that
$\dot{\mathbf{v}} = G\mathbf{v} + (\text{membrane terms})$. The
sign pattern of $G$ is the weighted graph Laplacian:
$G_{jk} = +\tilde g_{jk}$ for adjacent $k\neq j$,
$G_{jj} = -\sum_{k\neq j} \tilde g_{jk}$, and zero otherwise. Rows sum
to zero (current conservation), and $G$ is symmetric only when $c_j
A_j$ is uniform across compartments.
...
$[G\boldsymbol{\phi}]_j = \tilde g\,(\phi_{j-1} - 2\phi_j + \phi_{j+1})$
with raw axial conductance $g = \pi a^2 / (2\rho_a\,\Delta x)$ and
normalised entry $\tilde g = g/(c\,A)$, recovering the standard
